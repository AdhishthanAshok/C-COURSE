...
void insert(struct node* root , int key){ 
     // HERE THE SEARCHING SIMILIAR ALGO IS USED TO INSERT AN NODE........

          struct node * prev = NULL;    // PREV NODE WILL BE THE ROOT OF THE INSERT-CHILD
          while(root!=NULL){
            prev = root;
              if (key == root->data)
              {   printf("\nElement is already there ...no insertion possible.\n");
                 return ;
              }
              else if(key < root->data){
               root =  root->left;
    ...
    }             // HERE A NEW NODE IS MADE FROM THE CREATENODE FUNCTION
